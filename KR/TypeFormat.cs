...
namespace KR
{
    public partial class TypeFormat : Form
    {
        ...
        {
            dgw.Rows.Clear();
            string queryString = $"select * from Тип_номера_рекламного_формата";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string query = "INSERT INTO Тип_номера_рекламного_формата (Название) VALUES (@Name)";
        ...
        private void TypeFormat_Load(object sender, EventArgs e)
        {
            CreateColums();
        ...
        private void Search(DataGridView dgw)// поиск по записям
        {
            ...
            string searchString = $"select * from Тип_номера_рекламного_формата where concat (Номер_типа_рекламного_формата,Название) like '%" + textBoxSeacrh.Text + "%'";
        ...
        private void DeleteRow()
        {
            ...
            {
                MessageBox.Show("Невозможно удалить тип формата, так как он используется в других записях.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void Update()
        {
            ...
            for (int index = 0; index < dataGridView1.Rows.Count; index++)
            {
                ...
                if (rowState == RowState.Deleted)
                {
                    ...
                    var deleteQuery = $"delete from Тип_номера_рекламного_формата where Номер_типа_рекламного_формата = {id}";
                ...
                if (rowState == RowState.ModifiedNew)
                {
                    ...
                    var ChangeQuery = $"update Тип_номера_рекламного_формата set Название = '{Name}' where Номер_типа_рекламного_формата = '{id}'";
        ...
        private bool IsEmployeeUsed(int employeeId)
        {
            ...
            string queryString = $"SELECT COUNT(*) FROM Рекламные_каналы WHERE Номер_типа_рекламного_формата = {employeeId}";
